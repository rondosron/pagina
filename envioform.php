<?php
	...
	$presupuestoListform = $_POST['presupuestoListForm'];
	$totalResultForm = $_POST['totalResultForm'];
	...
	if(isset($_POST["submit"])){
		...
		// Phone and email validation (only ONE required)
			...
			if (count($errors) !== 0) {
				...
				$_SESSION['presupuestoListForm'] = $presupuestoListform; // To mantain/display value in index
				$_SESSION['totalResultForm'] = $totalResultForm; // To mantain/display value in index
				...
				unset($_SESSION['presupuestoListForm']);
				unset($_SESSION['totalResultForm']);
				$errors = [];
